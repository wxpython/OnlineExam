...
def _execute_task(task_id):
    ...
    try:
        ...
        try:
            ...
            other_mark = None
            if host:
                other_mark = OnlineMark(second_account, mark_pwd, host, log_callback=log)
                ...
                for h in all_hosts:
                    other_mark = OnlineMark(second_account, mark_pwd, h, log_callback=log)
                    if other_mark.login():
                        host = h
                        break
                    time.sleep(1.5)
            ...
            if lesson_id:
                other_mark.getQuestion(lesson_id, ques_id, merged_data, progress_callback=on_progress, refresh_delay=task.refresh_delay)
            else:
                log('二评账号未找到试题')
